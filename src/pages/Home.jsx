...
export default function Home() {
  ...
  const pages = ((array, numPerPage) => {
    const pages = [];
    for (let i = 0; i < array.length; i += numPerPage) {
      const smallArr = array.slice(i, i + numPerPage);
      if (smallArr.length !== 0) pages.push(smallArr);
    }
    return pages;
  })(countriesToBeRendered, cardsPerPage);
  ...
  const pagiBar = (
    <Stack spacing={1}>
      <Typography align="center">
        Showing {Math.min(cardsPerPage, countriesToBeRendered.length)} countries
        out of {countriesToBeRendered.length}
      </Typography>
      <Pagination
        count={pages.length}
        page={currentPage}
  ...
  const loadingMessage = useMemo(() => {
    if (loading)
      return <Typography align="center">Loading data from API...</Typography>;
